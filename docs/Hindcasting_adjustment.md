...
The output of the hindcasting adjusmtent procedure are two netCDF files, which are placed in the `FromModel` folder and can be directly read by Delft-FEWS. The first netCDF (`adjustment_factors_gridded_rainfall.nc`) contains the `additive` and `multiplier` adjustment variables, which can be applied to the original rainfall field as:

$$
P_{adj}(m,n) = P_{orig}(m,n) \cdot multiplier + additive.
$$
...
The way the gauge adjustment operates (e.g., maximum change factors, the settings for the adjustment methods, minimum thresholds, etc.) can be configured in `adjustment_settings.xml`. All possible settings in the xml file are describe in [/config/README.md](https://github.com/Deltares-research/pyRainAdjustment/blob/main/config/README.md).
